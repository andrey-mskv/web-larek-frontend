...
// Проверяет является ли переданный аргумент строкой, и эта строка имеет длину больше 1 символа
...
// Предназначена для того, чтобы гарантировать наличие одного элемента DOM, независимо от того, что передано — строка-селектор,
// коллекция элементов или сам элемент. Если селектор возвращает несколько элементов или ничего, функция выводит предупреждения или генерирует ошибку.
...
/**
 * Функция предназначена для клонирования первого дочернего элемента из шаблона (<template>),
 * который можно получить через селектор или переданный элемент шаблона.
 */
...
/**
 * создания CSS-классов в стиле методологии BEM
 */
export function bem(
	block: string,
	element?: string,
...
/**
 * Получения списка имён свойств объекта, с возможностью применения фильтра для их выбора
 */
...
export function insertSpaces(str: string | null) {
	if (str.length <= 4) {
		return str;
	} else {
		const reversStr = str.split('').reverse().join('');
		const groups = reversStr.match(/.{1,3}/g);
